...
## Bug-shape templates for `tech-debt-inventory` (3)
...
- [coveragepy] **CRF-COVPY-0017** [FIX] sysmon re-reads source from disk at measurement time and swallows tokenize errors — `sysmon.py:489-503` · shape `measurement-depends-on-mutable-external-state`
- [coveragepy] **CRF-COVPY-0039** [CONSIDER] timid = True silently discards an explicit core = setting — `core.py:83-89` · shape `conflict-resolved-silently-where-siblings-warn`
- [coveragepy] **CRF-COVPY-0044** [POLICY] sysmon retains every code object for the process lifetime — `sysmon.py:213-215, :372` · shape `identity-by-id-requires-retention`

Shapes represented above, in catalog terms: `conflict-resolved-silently-where-siblings-warn`, `identity-by-id-requires-retention`, `measurement-depends-on-mutable-external-state`
